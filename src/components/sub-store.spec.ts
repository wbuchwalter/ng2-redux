...
describe('Substore', () => {
  ...
  it('adds a key to actions it dispatches', () =>
    expect<any>(subStore.dispatch({ type: 'MY_ACTION' })).toEqual({
      type: 'MY_ACTION',
  ...
  it('handles property selection on a base path that doesn\'t exist yet', () => {
    ...
    nonExistentSubStore
      ...
      .take(2)
      .toArray()
      .subscribe(v => expect<any>(v).toEqual([undefined, 'now I exist']));
  ...
  it('can create its own sub-store', () => {
    ...
    expect<any>(subSubStore.dispatch({ type: 'MY_ACTION' })).toEqual({
      type: 'MY_ACTION',
